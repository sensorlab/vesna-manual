...
Set the make target name to \verb+all+.
    ...
    \begin{figure}[H]
    \centering
        \includegraphics[width=0.8\textwidth]{./install-guide-linux-images/compile-add-target.png}
        \caption{Adding a make target}
        \label{fig:compile-add-target}
    \end{figure}

Other useful targets are \verb+clean+ for cleaning the source code from the
build output and \verb+drivers_demo.load5+ for loading the application
to VESNA.
